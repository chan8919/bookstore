...
const join = async (req, res) => {
    ...
    try {
        ...
        const exist = await memberModel.isExistByEmail(conn, inputData.email);
        if (exist) {
            res.status(StatusCodes.BAD_REQUEST).json({ "err": "이미 존재하는 email 입니다." });
            return;
        };  // status 200 으로 보내는게 맞을까? 4xx 대로 보내야하나? 고민해보자
        // 회원 등록
        await memberModel.addMember(conn, { 'email': inputData.email, 'name': inputData.name, 'pwd': inputData.pwd });
...
const resetRequestPassword = async (req, res) => {
    ...
    try {
        // DB 커넥션 생성성
        conn = await dbPool.getConnection();
        const isExist = await memberModel.isExistByEmail(conn,inputData.email);
        if (isExist) {
...
const resetPassword = async (req, res) => {
    ...
    try {
        ...
        const isExist = await memberModel.isExistByEmail(conn,inputData.email);
        if (isExist) {
            await memberModel.resetPassword(conn,inputData.email, inputData.pwd);
